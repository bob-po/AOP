...
| Agents × 8 | Python | `8001`–`8008` | search/rag/report/analysis/image/video/code/browser |
...
脚本会用 `uvicorn` 在 `8001`–`8008` 拉起 8 个 agent，健康检查通过后向 Gateway 注册，最后打印注册数量与 `ALL AGENTS READY`。
...
- [database.md](./database.md) — 表结构与迁移
- [redis.md](./redis.md) — 队列/锁
- [a2a.md](./a2a.md) / [agent.md](./agent.md) — 协议与 Agent 规范
