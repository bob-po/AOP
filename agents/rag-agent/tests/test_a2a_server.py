"""A2A OS: RAG Agent as Server + Client (lineage, delegation, cancel)."""
...
def test_message_send_still_works_without_os(client):
    ...
    resp = _send(test_client, "User goal: what is a2a")
...
def test_autonomous_delegation_folds_into_metadata(client, monkeypatch):
    ...
    collab = AgentCollaborator(agent_id="rag-agent", delegate_skill="business-analysis", runtime=FakeRuntime())
