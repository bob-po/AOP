...
from context import parse_composed_query
from pipeline.compose import build_report
...
try:
    ...
    _COLLAB = AgentCollaborator(agent_id="report-agent")
...
app = FastAPI(title="AOP Report Agent", version="0.4.0")
...
def _skill_id(params: dict[str, Any]) -> str:
    ...
    skill = metadata.get("skillId") or metadata.get("skill_id") or "report-generation"
    return str(skill)


def run_report(query: str, *, skill: str = "report-generation") -> dict[str, Any]:
    parsed = parse_composed_query(query)
    return build_report(parsed["goal"], parsed["upstream"], skill=skill)
...
def _completed_task(task_id: str, query: str, payload: dict[str, Any]) -> dict[str, Any]:
    task = {
        ...
        "artifacts": [
            {
                ...
                "name": "report-summary",
                "description": "Report plan summary",
        ...
        "metadata": {"skillId": payload.get("skill") or "report-generation", "query": query},
    }
    _TASKS[task_id] = task
    return task
...
async def a2a_rpc(request: Request) -> JSONResponse:
    ...
    try:
        ...
        if method == "message/send":
            ...
            payload = run_report(query, skill=skill)
