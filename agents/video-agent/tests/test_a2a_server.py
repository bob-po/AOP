"""A2A OS: Video Agent as Server + Client (lineage, delegation, cancel)."""
...
def test_message_send_still_works_without_os(client):
    ...
    resp = _send(test_client, "30s promo video for the A2A OS platform")
...
def test_tasks_cancel(client):
    test_client, agent = client
    task_id = _send(test_client, "cancel me video").json()["result"]["id"]
...
def test_autonomous_delegation_folds_into_metadata(client, monkeypatch):
    ...
    collab = AgentCollaborator(agent_id="video-agent", delegate_skill="business-analysis", runtime=FakeRuntime())
    collab._enabled_override = True
    monkeypatch.setattr(agent, "_COLLAB", collab)

    resp = _send(test_client, "deep topic video", correlationId="c1", rootTaskId="r1", depth=1)
    result = resp.json()["result"]
