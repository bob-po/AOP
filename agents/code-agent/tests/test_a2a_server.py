"""A2A OS: Code Agent as Server + Client (lineage, delegation, cancel)."""
...
def test_tasks_cancel(client):
    test_client, agent = client
    task_id = _send(test_client, "sum([1, 2, 3])").json()["result"]["id"]
...
def test_autonomous_delegation_folds_into_metadata(client, monkeypatch):
    ...
    collab = AgentCollaborator(agent_id="code-agent", delegate_skill="business-analysis", runtime=FakeRuntime())
    collab._enabled_override = True
    monkeypatch.setattr(agent, "_COLLAB", collab)

    resp = _send(test_client, "sqrt(16) + 1", correlationId="c1", rootTaskId="r1", depth=1)
    result = resp.json()["result"]
