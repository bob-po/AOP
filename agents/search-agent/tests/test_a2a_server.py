"""A2A OS: Search Agent as Server + Client (lineage, delegation, cancel)."""
...
def test_message_send_still_works_without_os(client):
    ...
    resp = _send(test_client, "User goal: multi-agent orchestration patterns")
...
def test_autonomous_delegation_folds_into_metadata(client, monkeypatch):
    ...
    collab = AgentCollaborator(agent_id="search-agent", delegate_skill="business-analysis", runtime=FakeRuntime())
