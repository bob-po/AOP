"""PPT Agent — DeepPresenter (PPTAgent) backend with python-pptx stub fallback."""
...
try:
    ...
    _COLLAB = AgentCollaborator(agent_id="ppt-agent")
...
app = FastAPI(title="AOP PPT Agent", version="0.1.0")
        ...
        "artifacts": [
            {
                ...
                "name": "ppt-summary",
                "description": "PPT generation summary",
                "parts": [{"type": "text", "text": payload.get("content") or ""}],
            },
            {
                "artifactId": str(uuid.uuid4()),
                "name": "ppt-data",
...
async def health() -> dict[str, str]:
    return {
        "status": "ok",
        "agent": "ppt-agent",
        "ppt_mode": (os.getenv("PPT_MODE") or "auto"),
    }
